...
export const pythonCampaign = createCampaign({
  ...
  chapters: [
    ...
    {
      id: "python-chapter-6",
      title: "Modulo 6: Mini-projeto Python",
      description: "Crie um pequeno gerenciador de tarefas com listas, funcoes e arquivo.",
      order: 6,
      stages: [
        stage({ id: "py-project-01-model", order: 1, title: "Modelo de tarefa", description: "Represente uma tarefa como dicionario.", type: "challenge", xp: 100, coins: 55, content: [intro("Dados antes de tela", "Projetos ficam mais simples quando voce escolhe uma estrutura clara. Aqui, cada tarefa sera um dicionario com texto e status."), example("Uma tarefa", "O booleano diz se ela foi concluida.", "tarefa = {\"texto\": \"Estudar Python\", \"concluida\": False}", "tarefa criada"), challenge("Tarefa inicial", "Crie tarefa com texto \"Ler aula\" e concluida False.", "tarefa = {\"texto\": \"Ler aula\", \"concluida\": False}", "Use um dicionario com duas chaves.")]}),
        stage({ id: "py-project-02-add", order: 2, title: "Adicionar com funcao", description: "Use funcao para centralizar uma regra.", type: "challenge", xp: 115, coins: 65, content: [intro("Uma porta de entrada", "Quando toda adicao passa por uma funcao, fica facil validar texto vazio e manter o formato certo."), example("Adicionar tarefa", "strip remove espacos antes de validar.", "def adicionar(tarefas, texto):\n    limpo = texto.strip()\n    if limpo:\n        tarefas.append({\"texto\": limpo, \"concluida\": False})", "lista atualizada"), challenge("Adicionar produto", "Crie adicionar_produto(produtos, nome) que usa strip e adiciona {\"nome\": limpo} se limpo existir.", "def adicionar_produto(produtos, nome):\n    limpo = nome.strip()\n    if limpo:\n        produtos.append({\"nome\": limpo})", "Use strip, if limpo e append.")]}),
        stage({ id: "py-project-03-list", order: 3, title: "Listar com enumerate", description: "Mostre itens numerados para o usuario escolher.", type: "challenge", xp: 110, coins: 60, content: [intro("Indice amigavel", "enumerate permite mostrar 1, 2, 3 para o usuario mesmo que a lista use indices 0, 1, 2 por dentro."), example("Listar tarefas", "start=1 faz a contagem comecar em 1.", "for indice, tarefa in enumerate(tarefas, start=1):\n    print(f\"{indice}. {tarefa['texto']}\")", "tarefas numeradas"), challenge("Listar produtos", "Use enumerate(produtos, start=1) e imprima indice e produto['nome'].", "for indice, produto in enumerate(produtos, start=1):\n    print(f\"{indice}. {produto['nome']}\")", "Use f-string com indice e nome.")]}),
        stage({ id: "py-project-04-save", order: 4, title: "Salvar dados simples", description: "Grave uma linha por item em arquivo txt.", type: "challenge", xp: 120, coins: 70, content: [intro("Persistencia basica", "Salvar em arquivo permite que o programa gere um resultado fora da memoria. Para comecar, uma linha por item ja resolve muito."), example("Salvar nomes", "O \\n quebra linha entre registros.", "with open(\"tarefas.txt\", \"w\") as arquivo:\n    for tarefa in tarefas:\n        arquivo.write(tarefa[\"texto\"] + \"\\n\")", "arquivo salvo"), challenge("Salvar produtos", "Grave cada produto['nome'] em produtos.txt, uma linha por produto.", "with open(\"produtos.txt\", \"w\") as arquivo:\n    for produto in produtos:\n        arquivo.write(produto[\"nome\"] + \"\\n\")", "Use with open no modo w e percorra produtos.")]}),
        stage({ id: "py-project-05-boss", order: 5, title: "Chefe final: Gerenciador de tarefas", description: "Junte modelo, funcao, listagem e salvamento.", type: "boss", xp: 270, coins: 155, content: [intro("Projeto fechado", "Agora voce monta uma funcao pequena, mas com cara de app: recebe dados, valida, adiciona e persiste."), boss("Adicionar e salvar", "Crie registrar(tarefas, texto) que chama adicionar(tarefas, texto) e depois salva as tarefas em tarefas.txt.", "def registrar(tarefas, texto):\n    adicionar(tarefas, texto)\n    with open(\"tarefas.txt\", \"w\") as arquivo:\n        for tarefa in tarefas:\n            arquivo.write(tarefa[\"texto\"] + \"\\n\")", "Use a funcao adicionar e depois with open para salvar.")]}),
      ],
    },
  ],
});
